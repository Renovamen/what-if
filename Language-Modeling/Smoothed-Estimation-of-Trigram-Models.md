...
综上，我们引入了三个平滑参数 $$\lambda_1,\lambda_2,\lambda_3$$，这三个参数可以被理解为是三元、二元、一元极大似然估计的置信度或权重。比如，如果 $$\lambda_1$$ 的值非常接近于 1 ，则意味着我们认为 $$q_{ML}(w|u,v)$$ 的意义很大；相反，如果 $$\lambda_1$$ 接近于 0，那么我们认为 $$q_{ML}(w|u,v)$$ 的意义不大。

在实际操作中，我们应该让 $$\lambda_1,\lambda_2,\lambda_3$$ 根据不同的二元组 $$(u, v)$$ 进行改变。当 $$c(u, v)$$ 更大时，我们应该将 $$\lambda_1$$ 变得更大。因为我们认为当 $$c(u, v)$$ 较大时，应该给三元估计更高的置信度。
...
$$
\begin{aligned}
   \Pi(u,v)=1 & \ \ \ \ \ \text{if } 100 \leq c(u,v) \\
   \Pi(u,v)=2 & \ \ \ \ \ \text{if } 50 \leq c(u,v) < 100 \\
   \Pi(u,v)=3 & \ \ \ \ \ \text{if } 20 \leq c(u,v) < 50 \\
   \Pi(u,v)=4 & \ \ \ \ \ \text{if } 10 \leq c(u,v) < 20 \\
   \Pi(u,v)=5 & \ \ \ \ \ \text{if } 5 \leq c(u,v) < 10 \\
   \Pi(u,v)=6 & \ \ \ \ \ \text{if } 2 \leq c(u,v) < 5 \\
   \Pi(u,v)=7 & \ \ \ \ \ \text{if } c(u,v) = 1 \\
   \Pi(u,v)=8 & \ \ \ \ \ \text{if } c(u,v) = 0 \text{ and } c(v) > 0 \\
   \Pi(u,v)=9 & \ \ \ \ \ \text{otherwise}
\end{aligned}
$$
