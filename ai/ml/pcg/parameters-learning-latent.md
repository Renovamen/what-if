...
如果图模型中包含隐变量，即有部分变量是不可观测的，这时就需要用 **EM 算法（Expectation Maximum，期望最大化算法）**来进行参数估计。
...
- M 步：固定 $$q_{t+1} (z)$$，找到一组参数 $$\theta_{t+1}$$，使得 $$\text{ELBO}_{\theta_{t+1}}(q_{t+1}, x) ≥ \text{ELBO}_{\theta_t}(q_{t+1}, x)$$
